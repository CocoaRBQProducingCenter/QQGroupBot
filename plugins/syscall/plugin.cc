...
EventProcessEnum OnGroupMessage(GroupMessageData data)
{
    ...
    api->OutputLog("Judge MessageClip");
    // 判断是否是长消息自动分片的片段内容（序列从0开始）
    if (data.MessageClipID > 0 && data.MessageClip + 1 != data.MessageClipCount)
    {
        api->OutputLog("syscall msg_clip Ignore");
        // 不处理长消息自动分片的片段内容
        return EventProcessEnum::Ignore;
    }

    api->OutputLog("Judge SenderQQ");
    // 判断是否是自己发送的消息
    if (data.ThisQQ == data.SenderQQ)
    {
        api->OutputLog("syscall ac==from Block");
        // 不处理自己发送的消息
        return EventProcessEnum::Block;
    }

    api->OutputLog("Judge MessageType");
    // 判断消息类型，只处理普通群聊信息
    if (data.MessageType != MessageTypeEnum::GroupUsualMessage)
    {
        api->OutputLog("syscall msg_type Ignore");
        // 不处理其他消息
        return EventProcessEnum::Ignore;
    }
    nlohmann::json J;
    ...
    try
    {
        if (ReceiveMessage(data.ThisQQ, data.SenderQQ, data.MessageGroupQQ, retransCode(data.MessageContent).c_str(), data.MessageReceiveTime, 1))
        {
            api->OutputLog("syscall Syscall Block");
            return EventProcessEnum::Block;
        }
    }
    catch (exception e)
    {
        MessageBoxW(nullptr, UTF8ToWideChar(e.what()).c_str(), UTF8ToWideChar("Try Judge Contexts Error").c_str(), MB_OK | MB_ICONERROR);
    }
    try
    {
        api->OutputLog("syscall final Ignore");
    }
    catch (exception e)
    {
        MessageBoxW(nullptr, UTF8ToWideChar(e.what()).c_str(), UTF8ToWideChar("Try Final Output Error").c_str(), MB_OK | MB_ICONERROR);
    }
    // 已经处理过的消息阻止其他插件继续处理
    return EventProcessEnum::Ignore;
}
